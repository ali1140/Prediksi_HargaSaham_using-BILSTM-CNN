...
from tensorflow.keras.layers import LSTM, Dense, Dropout, Conv1D, MaxPooling1D, Bidirectional # Ditambahkan Conv1D, MaxPooling1D, Bidirectional
from sklearn.preprocessing import MinMaxScaler
import os
import joblib # Untuk menyimpan scaler
import glob # Untuk mencari file CSV
import matplotlib.pyplot as plt 

# --- Fungsi Helper ---
def convert_volume_to_numeric(volume_str):
    """Mengonversi string volume (misalnya '1.5M', '100K') menjadi angka."""
    ...
    if not volume_str: # Handle empty string
        return np.nan
    ...
    for suffix, multiplier in multipliers.items():
        if volume_str.endswith(suffix):
            try:
                ...
                return np.nan
    try:
        return float(volume_str) # Jika sudah angka tanpa suffix
    except ValueError:
        return np.nan

def convert_change_pct_to_numeric(change_str):
    """Mengonversi string persentase perubahan (misalnya '5.25%') menjadi float."""
    ...
    if not change_str: # Handle empty string
        return np.nan
...
def create_sequences_multi_feature(data, sequence_length, future_steps, target_column_index):
    """
    Membuat sekuens input (X) dan output (y) untuk model time series multi-fitur.
    """
    X, y = [], []
...
def train_and_save_model_for_stock(stock_name, df_stock_raw, feature_columns, target_column, 
    ...
    """
    Melatih model CNN-BiLSTM untuk satu saham dan menyimpannya, serta membuat plot.
    """
    ...
    if dates_to_exclude_str_list: 
        ...
        if not exclude_timestamps.empty:
            ...
            if not isinstance(df_processed.index, pd.DatetimeIndex):
                print(f"Peringatan: Indeks untuk {stock_name} bukan DatetimeIndex.")
            else:
    ...
    if len(df_features) < original_len:
        print(f"   {original_len - len(df_features)} baris dengan NaN dihapus untuk {stock_name}.")
    ...
    scaler_filename = os.path.join(scalers_dir, f"scaler_cnn_bilstm_{safe_stock_name}.gz") # Penamaan baru
    ...
    # --- PENYESUAIAN ARSITEKTUR MODEL: CNN + BiLSTM ---
    model = Sequential([
        Conv1D(filters=64, kernel_size=3, activation='relu', 
               input_shape=(sequence_length, num_features), padding='same'),
        # MaxPooling1D(pool_size=2), # Opsional: bisa ditambahkan jika ingin downsampling sekuens
        # Jika menggunakan MaxPooling, perhatikan perubahan panjang sekuens untuk layer LSTM berikutnya
        
        Bidirectional(LSTM(units=128, return_sequences=True)), # Layer BiLSTM pertama
        Dropout(0.40), 
        Bidirectional(LSTM(units=64, return_sequences=False)), # Layer BiLSTM kedua, return_sequences=False karena diikuti Dense
        Dropout(0.30), 
        # Tidak perlu LSTM(units=32) jika BiLSTM terakhir sudah return_sequences=False
        Dense(units=future_prediction_days) 
    ])

    optimizer = tf.keras.optimizers.Adam(learning_rate=0.0005) 
    model.compile(optimizer=optimizer, loss='mean_squared_error')
    model.summary() # Tampilkan summary model baru
    ...
    model_filename = os.path.join(models_dir, f"model_cnn_bilstm_{safe_stock_name}.h5") # Penamaan baru
